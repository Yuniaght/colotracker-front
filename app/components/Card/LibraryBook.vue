  ...
  <nuxt-link :to="{
    ...
    <div class="flex items-stretch min-h-[120px]">
      ...
      <div class="w-26 shrink-0 relative overflow-hidden bg-gray-100">
        <nuxt-picture provider="directus" loading="lazy"
          :src="`${item.book.front_cover.id}/${item.book.front_cover.filename_download}`"
          :alt="item.book.front_cover.title" class="absolute inset-0 w-full h-full"
    ...
    <div class="p-4">
      <p class="flex justify-between pb-1"><span>Progression</span><span>{{ calculateProgress(item?.completed_pages,
        item?.book.page_count) }}</span></p>
